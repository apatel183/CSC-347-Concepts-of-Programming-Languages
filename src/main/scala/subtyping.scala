...
object subtyping {
  ...
  class myTracker extends Counter {
  private var number = 0
  private var sum = 0
  override def increment () = { number = number + 1; sum +=1}
  override def decrement () = { number = number + 1; sum +=1}
  def getSum () : Int = sum
}
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    ...
    //List (-1, -1, -1)
    val tracker =  new myTracker
    val trackerI = new myTracker
    val trackerII = new myTracker
    f(List(tracker,trackerI,trackerII))
	List(tracker.getSum, trackerI.getSum, trackerII.getSum)
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    ...
    val trackerI = new myTracker
    val trackerII = new myTracker
    f(Array(tracker,trackerI,trackerII))
	Array(tracker.getSum, trackerI.getSum, trackerII.getSum)
  }
}
