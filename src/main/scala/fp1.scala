 ...
 * code immediately below the comment. Lalu
 *
...
object fp1 {
  ...
  def maxTailRecursiveAux (accumulator : Int, xs : List[Int]) : Int = { xs match {
    // TODO: Provide definition here.
    //-1
    case Nil => accumulator
    case y :: Nil => 
    val n_number = y
    if( n_number > accumulator)
      n_number 
    else
      accumulator
      case y :: ys => 
      val n_number1 = y
      if ( n_number1 > accumulator)
        maxTailRecursiveAux(n_number1, ys)
      else 
        maxTailRecursiveAux(accumulator,ys)
  } 
  }
