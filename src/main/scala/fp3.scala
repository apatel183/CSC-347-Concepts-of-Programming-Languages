...
object fp3 {
  ...
  def member (a : Int, xs : List[Int]) : Boolean = { xs match {
    ...
    //false
    case Nil => false 
    case b::xs => if (b == a) true else member(a,xs)
}
  }
  ...
  def allEqual (xs : List[Int]) : Boolean = { xs match {
     ...
     //false
     case Nil => true
     case a if (xs.length == 1) => true
     case a::xs => if (member(a,xs) != true ) false else allEqual(xs)
  }
  }
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = { ys match {
    ...
    //null
    case List() => List()
    case a::ys if (x == a) => (delete1(x,ys))
    case a::ys => a :: (delete1(x,ys))
}
  }
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    // TODO: Provide definition here.
    //null
    def auxiliary[X] (duplicateVal:X, ys:List[X]) : List[X]= ys match {
      case List() => {
      if (duplicateVal != null) duplicateVal :: List()
      else List()
      }
      case a::ys => {
      if (a != duplicateVal) duplicateVal :: auxiliary(a,ys) else auxiliary(a,ys)
      }
    }
    if (xs.length > 0) auxiliary(xs.head, xs.tail) else List()
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    //null
    def auxiliary[X] (duplicateVal: X, ys:List[X], count:Int) : List[(Int,X)] = ys match{
      case List() => {
      if (duplicateVal != null) (count,duplicateVal) :: List()
      else List()
      }
      case a :: ys => {
      if(a !=duplicateVal)(count,duplicateVal) :: auxiliary(a,ys,1)
      else auxiliary(a,ys, count+1)
      }
    }
    if (xs.length >0) auxiliary(xs.head, xs.tail, 1 ) 
    else List()
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    ...
    case (a, Nil)  => (Nil, Nil)
    case (-1, lst) => (List(), lst)
    case (n, x :: xs) => { val (a1, a2) = splitAt(n - 1, xs)
  ...
  def allDistinct (xs : List[Int]) : Boolean = { 
    ...
    //false
    case Nil => true
    case a::xs => if(member(a,xs) == true) false else allDistinct(xs)
  }
}
}
