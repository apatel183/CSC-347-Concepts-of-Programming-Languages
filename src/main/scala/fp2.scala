...
object fp2 {
  ...
  def filter [A] (xs:List[A], f:A=>Boolean) : List[A] = xs match {
    ...
    //null
    case List() => List()
    case (a :: xs) if (f (a)) => a :: (filter (xs,f)) 
    case (a :: xs) => filter(xs,f)
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    //null
    def f(a: String) (b: String, c: String) : String = b + c + a 
    xs match {
      case Nil  => ""
      case y :: ys => foldLeft(y::ys, "", f(term)) 
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    ...
    firstIndexNumGreaterThan(0,a,xs)
  }

  def firstIndexNumGreaterThan(b : Int, a : Int, xs : List[Int]) : Int = {
    if(xs.head < a)
      firstIndexNumGreaterThan(b + 1, a , xs.tail)
    else 
      b
  }
}
